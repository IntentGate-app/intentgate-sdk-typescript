# Releasing @netgnarus/intentgate
...
1. **Reserve the `@netgnarus` org.** Sign in to npmjs.com, go to *Add organization*, choose the name `netgnarus`, pick the free plan. Org names are first-come; pick this before any of the SDK release rehearsals.

2. **Create the package shell.** First publish of a scoped package needs `--access public` (the workflow does this), but the package name `@netgnarus/intentgate` must not already be squatted. Quick check:

   ```sh
   npm view @netgnarus/intentgate
...
3. **Configure the trusted publisher.** This is the actual OIDC wire-up. On the package's settings page (or, since the package doesn't exist yet, in the org-level *Trusted Publishers* settings), add:
   ...
   - **Organization or user**: `NetGnarus`
...
After all three are in place, `npm view @netgnarus/intentgate --json` still 404s (the package hasn't been published yet) but the publisher is registered and the next workflow run will succeed.
...
The workflow validates `package.json` matches the tag, runs the full lint/test/build gate, then attempts `npm publish --provenance --access public`. If the trusted-publisher config is correct, the publish succeeds and you can immediately `npm unpublish @netgnarus/intentgate@0.1.0-rc.0` to clean up. If the OIDC handshake fails, the publish step errors with a 401 referencing the trusted-publisher mismatch — fix the npm-side config and retry.
...
- npm: https://www.npmjs.com/package/@netgnarus/intentgate
- workflow run: https://github.com/NetGnarus/intentgate-sdk-typescript/actions/workflows/release.yml
...
- **One less GitHub secret.** `NPM_TOKEN` was the last secret that would have lived on `NetGnarus/intentgate-sdk-typescript`. Now there's nothing to leak.

Same pattern, same rationale, as the [Python SDK's PyPI trusted publishing](https://github.com/NetGnarus/intentgate-sdk-python/blob/main/.github/workflows/release.yml) shipped session 11.

...
- **Dependabot** alerts on every push, scanning npm + GitHub Actions for known CVEs. Security updates are auto-proposed as PRs.
...
Releases publish to npm via **OIDC trusted publishing** with `--provenance` attestation — no long-lived API tokens in CI. Each release derives from a specific Git tag.
